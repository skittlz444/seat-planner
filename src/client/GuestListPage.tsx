...
const GuestListPage = ({ layoutId, layouts, onLayoutChange, onBack }: Props) => {
  ...
  const renderMiniTable = (table: TableInfo, highlightGuestId: string) => {
    ...
    const rows = Math.ceil(Math.max(table.max_seats, table.guests.length, maxPos + 1) / 2);
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <label className="flex items-center gap-2 print:hidden cursor-pointer select-none">
              <div
                ...
                onKeyDown={(e) => { if ((e.key === " " || e.key === "Enter") && !e.repeat) { e.preventDefault(); setShowTabledOnly((v) => !v); } }}
